export async function processHandlers(vm, handlers, abortSignal) {
    const vmAny = vm;
...
function parseHandler(key, eventName) {
...
async function setupHandler(vm, parsed, abortSignal, abortControllers, propertyListeners) {
    const vmAny = vm;
    // Function to attach listener to an EventTarget
    const attachListener = (eventTarget) => {
        if (!eventTarget)
            return null;
        ...
        // Add event listener
        eventTarget.addEventListener(parsed.eventName, async (event) => {
            // Call the method
    ...
    const resolveEventTarget = (value) => {
        if (!value)
            return null;
