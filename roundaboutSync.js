import { ROUNDABOUT_READY } from './makeRoundaboutReady.js';
 ...
 * ```
 */
export function roundaboutSync(options, infractions) {
    const vm = (options.vm || {});
    const proto = Object.getPrototypeOf(vm);
    const cache = proto?.[ROUNDABOUT_READY];
    // Create propagator
    let propagator;
    if (vm.propagator) {
    ...
    if (cache) {
        ...
        for (const prop of propertiesToMonitor) {
            propagator.addEventListener(prop, (event) => {
                const propChangeEvent = event;
...
// ─── Internal helpers ────────────────────────────────────────────────────────
function setupRoundaboutReadyInterface(vm, abortController) {
    if (!vm.RAController) {
...
function storeMetadata(vm, weakRefConfig) {
...
function createPropertyChangeHandler(vm) {
    const processingQueue = new Map();
    const pendingValues = new Map();
    async function handlePropertyChange(key, value) {
    ...
    async function processPropertyChangeInternal(key, _value) {
...
function processOptionsSync(vm, options, modules, handlePropertyChange, abortController, infractions) {
    const onChange = (key) => handlePropertyChange(key, vm[key]);
    ...
    if (options.actions && modules.processActions) {
        modules.processActions(vm, options.actions, onChange, options.internalRouting === true);
    }
...
async function deferProcessorWiring(vm, options, handlePropertyChange, abortController, infractions) {
    const onChange = (key) => handlePropertyChange(key, vm[key]);
    const promises = [];
    if (options.compacts) {
        promises.push(import('./processors/compacts.js').then(({ processCompacts }) => {
            processCompacts(vm, options.compacts, onChange);
        }));
    }
    if (options.actions) {
        promises.push(import('./processors/actions.js').then(({ processActions }) => {
            processActions(vm, options.actions, onChange, options.internalRouting === true);
        }));
    }
    if (options.handlers) {
        promises.push(import('./processors/handlers.js').then(({ processHandlers }) => {
            processHandlers(vm, options.handlers, abortController.signal);
        }));
    }
    if (options.hitches) {
        promises.push(import('./processors/hitches.js').then(({ processHitches }) => {
            processHitches(vm, options.hitches, onChange);
        }));
    }
    if (options.positractions) {
        promises.push(import('./processors/positractions.js').then(({ processPositractions }) => {
    ...
    if (infractions) {
        promises.push(import('./processors/infractions.js').then(({ processInfractions }) => {
            processInfractions(vm, infractions, onChange);
        }));
    }
 ...
 */
function inferPropertiesToMonitorSync(options) {
    const props = new Set();
    ...
    if (options.actions) {
        for (const [, actionConfig] of Object.entries(options.actions)) {
...
function installGetterSettersInline(vm, propertiesToMonitor, propagator, weakRefConfig, isPlainObject) {
    if (isPlainObject) {
        const storage = {};
